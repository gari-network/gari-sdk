const { startTransactionsExternal } = require('./api.js')
 ...
 * @returns 
 */
async function initiateTransactionExternal(encodedTransaction, transactionData, jwtToken) {
    try {
        const validate = sdkValidate(`backend`)
        if (!validate) {
            throw new Error(`sdk not initialized`)
        }
        const transactionResponse = await startTransactionsExternal(encodedTransaction, transactionData, jwtToken)
...
module.exports = initiateTransactionExternal